...
When we compare the fits between the status quo ADMB model and the identically structured but correctly-specified version fit in TMB, we see large differences in model predictions. In particular the PE variance are very different, with the ADMB model producing a PE that is more than seven times the PE in the TMB model:
