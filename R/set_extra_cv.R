...
set_extra_cv <- function(input, extra_biomass_cv, extra_cpue_cv) {
  ...
  if(!is.null(extra_biomass_cv)) {
    ...
    if(extra_biomass_cv$assumption == 'extra_cv') {
      ...
      par$log_tau_biomass <- rep(0, length(unique(data$pointer_extra_biomass_cv)))
      map$log_tau_biomass <- as.factor(1:length(map$log_tau_biomass))
    }
  ...
  if(!is.null(extra_cpue_cv)){
    ...
    if(extra_cpue_cv$assumption == 'extra_cv') {
      ...
      par$log_tau_cpue <- rep(0, length(unique(data$pointer_extra_cpue_cv)))
      map$log_tau_cpue <- as.factor(1:length(map$log_tau_cpue))
    }
  ...
  if(!is.null(extra_biomass_cv$pointer_extra_biomass_cv)) {
    ...
    # data$extra_biomass_cv <- 1
    data$pointer_extra_biomass_cv <- (extra_biomass_cv$pointer_extra_biomass_cv)-1 # TMB starts indexing at 0
    # data$tau_biomass_upper <- rep(1.5, length(unique(data$pointer_extra_biomass_cv)))
    par$log_tau_biomass <- rep(0, length(unique(data$pointer_extra_biomass_cv)))
    map$log_tau_biomass <- par$log_tau_biomass
    map$log_tau_biomass <- as.factor(1:length(map$log_tau_biomass))
  }
  ...
  if(!is.null(extra_cpue_cv$pointer_extra_cpue_cv)) {
    ...
    # data$extra_cpue_cv <- 1
    data$pointer_extra_cpue_cv <- (extra_cpue_cv$pointer_extra_cpue_cv)-1 # TMB starts indexing at 0
    # data$tau_cpue_upper <- rep(1.5, length(unique(data$pointer_extra_cpue_cv)))
    par$log_tau_cpue <- rep(0, length(unique(data$pointer_extra_cpue_cv)))
    map$log_tau_cpue <- par$log_tau_cpue
    map$log_tau_cpue <- as.factor(1:length(map$log_tau_cpue))
  }

  # user defined initial values for log_tau_biomass parameters
  if(!is.null(extra_biomass_cv$initial_pars)) {
    if(length(extra_biomass_cv$initial_pars) != length(par$log_tau_biomass)) stop("extra_biomass_cv$initial_pars must be a vector with length equal to the number of extra biomass cv (log_tau_biomass) parameters in the model.  Input values must be numeric and users should specify log_tau_biomass initial values in log space.")
    if(any(!is.numeric(extra_biomass_cv$initial_pars))) stop("extra_biomass_cv$initial_pars must be a vector with length equal to the number of extra biomass cv (log_tau_biomass) parameters in the model.  Input values must be numeric and users should specify log_tau_biomass initial values in log space.")
    par$log_tau_biomass <- extra_biomass_cv$initial_pars
  }

  # user defined initial values for log_tau_cpue parameters
  if(!is.null(extra_cpue_cv$initial_pars)) {
    if(length(extra_cpue_cv$initial_pars) != length(par$log_tau_cpue)) stop("extra_cpue_cv$initial_pars must be a vector with length equal to the number of extra cpue cv (log_tau_cpue) parameters in the model.  Input values must be numeric and users should specify log_tau_cpue initial values in log space.")
    if(any(!is.numeric(extra_cpue_cv$initial_pars))) stop("extra_cpue_cv$initial_pars must be a vector with length equal to the number of extra cpue cv (log_tau_cpue) parameters in the model.  Input values must be numeric and users should specify log_tau_cpue initial values in log space.")
    par$log_tau_cpue <- extra_cpue_cv$initial_pars
  }

  # user defined fixed values of log_tau_biomass
  if(!is.null(extra_biomass_cv$fix_pars)) {
    if(any(!is.numeric(extra_biomass_cv$fix_pars))) stop("extra_biomass_cv$fix_pars must be a vector of integer value(s) with the index value (starting at 1) of the log_tau_biomass to be fixed.")
    extra_biomass_cv$fix_pars <- as.integer(extra_biomass_cv$fix_pars)
    if(any(!is.integer(extra_biomass_cv$fix_pars))) stop("extra_biomass_cv$fix_pars must be a vector of integer value(s) with the index value (starting at 1) of the log_tau_biomass to be fixed.")

    # create log_tau_biomass map for fixed/estimated parameters
    tmp_log_tau_biomass <- par$log_tau_biomass
    icounter <- 1
    for(i in 1:length(tmp_log_tau_biomass)) {
      if(i %in% extra_biomass_cv$fix_pars) {
        tmp_log_tau_biomass[i] <- NA
      } else {
        tmp_log_tau_biomass[i] <- icounter
        icounter <- icounter + 1
      }
    }
    map$log_tau_biomass <- as.factor(tmp_log_tau_biomass)
  }

  # user defined fixed values of log_tau_cpue
  if(!is.null(extra_cpue_cv$fix_pars)) {
    if(any(!is.numeric(extra_cpue_cv$fix_pars))) stop("extra_cpue_cv$fix_pars must be a vector of integer value(s) with the index value (starting at 1) of the log_tau_cpue to be fixed.")
    extra_cpue_cv$fix_pars <- as.integer(extra_cpue_cv$fix_pars)
    if(any(!is.integer(extra_cpue_cv$fix_pars))) stop("extra_cpue_cv$fix_pars must be a vector of integer value(s) with the index value (starting at 1) of the log_tau_cpue to be fixed.")

    # create log_tau_cpue map for fixed/estimated parameters
    tmp_log_tau_cpue <- par$log_tau_cpue
    icounter <- 1
    for(i in 1:length(tmp_log_tau_cpue)) {
      if(i %in% extra_cpue_cv$fix_pars) {
        tmp_log_tau_cpue[i] <- NA
      } else {
        tmp_log_tau_cpue[i] <- icounter
        icounter <- icounter + 1
      }
    }
    map$log_tau_cpue <- as.factor(tmp_log_tau_cpue)
  }
