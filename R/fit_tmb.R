...
#'   optimization. Default = \code{0}.
#' @param do.sdrep T/F, calculate standard deviations of model parameters? See
#'   \code{\link[TMB]{TMB::sdreport}}. Default = \code{TRUE}.
#' @param do.check T/F, check if model parameters are identifiable? Runs
#'   internal \code{check_estimability}, originally provided by
#'   https://github.com/kaskr/TMB_contrib_R/TMBhelper. Default = \code{TRUE}.
#' @param save.sdrep T/F, save the full \code{\link[TMB]{TMB::sdreport}} object?
#'   If \code{FALSE}, only save
#'   \code{\link[TMB:summary.sdreport]{summary.sdreport)}} to reduce model
#'   object file size. Default = \code{FALSE}.
#' @return \code{model}, appends the following: \describe{
#'   \item{\code{model$opt}}{Output from
#'   \code{\link[stats:nlminb]{stats::nlminb}}}
#'   \item{\code{model$date}}{System date} \item{\code{model$dir}}{Current
#'   working directory}
...
#'   \item{\code{model$sdrep}}{Estimated standard deviations for model
#'   parameters, \code{\link[TMB:sdreport]{TMB::sdreport}} or
#'   \code{\link[TMB:summary.sdreport]{summary.sdreport)}}} }
#'
#' @seealso \code{\link{fit_rema}}, \code{TMBhelper::check_estimability}
#'
fit_tmb = function(model, n.newton=0, do.sdrep=TRUE, do.check=FALSE, save.sdrep=FALSE)
{
