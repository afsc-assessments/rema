...
set_tweedie <- function(input, zeros) {
  ...
  if(zeros$assumption != 'tweedie' & !is.null(zeros$options_tweedie)) {
    stop("User has defined options_tweedie but did not define assumption = 'tweedie'. Please assumption = 'tweedie' in order to use the Tweedie distribution for the observation error in the model, or remove the options_tweedie.")
  }
